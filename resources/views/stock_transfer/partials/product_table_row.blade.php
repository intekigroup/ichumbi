        ...
        {{$product->sub_sku}}
            ...
            @if($product->enable_stock == 1)
                ...
                <small class="text-muted" style="white-space: nowrap;">
                    @lang('lang_v1.location_from') @lang('report.current_stock'):
                    <span class="qty_available_text">{{$product->formatted_qty_available}}</span> {{ $product->unit }}
                </small>
                <br>
                <small class="text-muted" style="white-space: nowrap;">
                    @lang('lang_v1.location_to') @lang('report.current_stock'):
                    <span class="qty_available_to_text">{{ $product->formatted_transfer_qty_available ?? 0 }}</span> {{ $product->unit }}
                </small>
            @endif
